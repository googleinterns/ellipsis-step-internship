...
class GetDataset(beam.DoFn):
    ...
    def process(self, element, image_provider):
        ...
        query = self.db.collection_group(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS)\
            .where(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS_FIELD_PIPELINE_RUN_ID, u'==', image_provider)\
            .where(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS_FIELD_RANDOM, u'>=', random_min)\
            .where(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS_FIELD_RANDOM, u'<', random_max)\
            .stream()
...
class UpdateVisibilityInDatabase(beam.DoFn):
    ...
    def process(self, element):
        image_doc_dict = element
        parent_image_ref = self.db.collection(database_schema.COLLECTION_IMAGES).\
            document(image_doc_dict[database_schema.COLLECTION_IMAGES_SUBCOLLECTION_PIPELINE_RUNS_FIELD_PARENT_IMAGE_ID])
        image_ref = element['id']
        self._update_visibility(parent_image_ref, image_doc_dict['id'], image_label_ids)
        self._update_parent_visibility(parent_image_ref, image_label_ids)

    def visibility(self, parent_image_ref, doc_id, label_id_list):
        ...
        doc_ref.update({
            database_schema.COLLECTION_IMAGES_SUBCOLLECTION_PIPELINE_RUNS_FIELD_VISIBILITY: database_schema.LABEL_VISIBILITY_VISIBLE
        })
        doc_ref.set({
    ...
    def _update_parent_image_labels_array(self, image_doc_ref, label_ids):
        """ Adds the label id to the image's document if it is not already there.
            ...
            Args:
                parent_image_ref: reference to the parent image document in the database
