...
def get_redefine_map(recognition_provider_id):
    db = initialize_db()
    doc_dict = db.collection(database_schema.REDEFINE_MAPS_COLLECTION_NAME).\
        document(recognition_provider_id).get().to_dict()
    return doc_dict[database_schema.REDEFINE_MAP]
...
class RedefineLabels(beam.DoFn):
    ...
    def process(self, element, redefine_map):
        ...
        """
        label_name = element[database_schema.LABEL_NAME]
