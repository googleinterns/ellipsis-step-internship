...
from backend_jobs.recognition_pipeline.pipeline_lib.firestore_database import RANGE_OF_BATCH, add_id_to_dict
...
class GetBatchedLabelsDataset(beam.DoFn):
    ...
    def process(self, element, recognition_run):
        ...
        query = self.db.collection_group(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS)\
            .where(database_schema.PIPELINE_RUN_ID, u'==', recognition_run).\
                    where(database_schema.RANDOM, u'>=', random_min).where(database_schema.RANDOM, u'<', random_max).stream()
...
class UpdateDatabase(beam.DoFn):
    ...
    def process(self, element):
        parent_image_ref = self.db.collection(database_schema.COLLECTION_IMAGES).document(element[0][database_schema.PARENT_IMAGE_ID])
        self.update_label_doc(parent_image_ref, element[0]['id'], element[1])
        self.update_parent_image_labels_array(parent_image_ref, element[1])

    def update_label_doc(self, parent_image_ref, doc_id, label_id_list):
        ...
        doc_ref = parent_image_ref.collection(database_schema.COLLECTION_IMAGES_SUBCOLLECTION_LABELS).document(doc_id)
        doc_ref.update({
            database_schema.VISIBILITY: database_schema.LABEL_VISIBILITY_VISIBLE
        })
        doc_ref.set({
            database_schema.LABEL_IDS: label_id_list
        }, merge = True)

    def update_parent_image_labels_array(self, image_doc_ref, label_ids):
        ...
        labels_array = []
        if database_schema.LABELS in image_doc_dict:
            labels_array = image_doc_dict[database_schema.LABELS]
        for label_id in label_ids:
            label_name = id_to_name(label_id) # TODO: after demo - change to ids not names.
            if label_name not in labels_array:
                labels_array.append(label_name)
            image_doc_ref.update({
            database_schema.LABELS: labels_array
        })

def id_to_name(label_id): # This method is temp - only for the demo
    labeltag_doc_ref = initialize_db().collection(database_schema.LABELTAGS_COLLECTION_NAME).\
        document(label_id)
...
def get_provider_id_from_run_id(run_id):
    ...
    return run_doc_dict[database_schema.PROVIDER_ID]
