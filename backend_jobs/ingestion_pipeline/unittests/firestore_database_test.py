...
class TestFlickrProvider(unittest.TestCase):
    ...
    def test_get_geo_hashes_map1(self):
        ...
        hashes_map = firestore_database.get_geo_hashes_map({'latitude':37.783371,'longitude':-122.439687})
        self.assertEqual(expected_hashes_map, hashes_map['hash10'])
 
    def test_get_geo_hashes_map2(self):
        expected_hashes_map = '7zzzzzzzzz'
        hashes_map = firestore_database.get_geo_hashes_map({'latitude':0.0,'longitude':0.0})
        self.assertEqual(expected_hashes_map, hashes_map['hash10'])
 
    def test_get_geo_hashes_map3(self):
        expected_hashes_map = 'pbpbpbpbpb'
        hashes_map = firestore_database.get_geo_hashes_map({'latitude':-90,'longitude':180})
        self.assertEqual(expected_hashes_map, hashes_map['hash10'])
    
    def test_get_date_fields(self):
        expected_date = {'year':2019,'month':4,'day':13}
        date = firestore_database.get_date_fields(datetime.date(2019, 4, 13))
